...
class MedicalToJson:
    def __init__(self):
        cur_dir = './'
        self.data_path = os.path.join(cur_dir, 'prepare_data/medical.json')
    ...
    def write_node_json(self, label, nodes):
        """
        Description:将实体建立成json文件，分开medical.json
        """
        count = 0
        en_list=[]
    ...
    def write_rel_json(self, start_node, end_node, edges, rel_type, rel_name,postfix=None):
        ...
        if postfix is None:
            filename='./data/'+rel_type+'.json'
        else:
            filename = './data/' + rel_type + postfix+'.json'
